        ...
        _(batch).each(function(metric) {
            _(metric.Dimensions).each(function(devices, region) {
                _(devices).each(function(device) {
                    var data;
                    if (results[z].Error) {
                        console.error('Retrieval error. Error %s', results[z].Error.Code);
                    } else {
                        data = results[z].GetMetricStatisticsResult.Datapoints.member;
                    }
                    if (data) {
                        payload[y] = {
                            name: function() {
                                if (metric.Name) {
                                    return _(metric.Name).template({
                                        region: region,
                                        metric: results[z].GetMetricStatisticsResult.Label,
                                        statistic: metric.Statistic,
                                        unit: metric.Unit,
                                        period: metric.Period
                                    });
                                } else {
                                    return results[z].GetMetricStatisticsResult.Label;
                                }
                            }(),
